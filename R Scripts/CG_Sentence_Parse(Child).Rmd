---
title: "CG_HCP_preprocess19May2020"
author: "E.T. Moseley"
date: "5/19/2020"
...
#write.csv(head(res[res$label_var == "Spouse/Partner",], 25), file = "~/Desktop/Spouse_examples.csv", row.names = F)

#write.csv(head(res[res$label_var == "Child",], 25), file = "~/Desktop/child_examples.csv", row.names = F)
```
...
any(res_train$sentence_source %in% res_test$sentence_source)
```
...
write.csv(res_train, file = "~/Desktop/CG_Data/learning_curves/balanced_child_train19May2020.csv", row.names = F)
write.csv(res_test, file = "~/Desktop/CG_Data/learning_curves/balanced_child_test19May2020.csv", row.names = F)
```
...
for (nm in unique(res_train$sentence_source)){
    ...
    if (length(out_files) %% 5 == 0){
            write.csv(tmp, file = paste("~/Desktop/CG_Data/learning_curves/child_train_", length(out_files), '_notes.csv', sep = ''), row.names=F)
    }
}
